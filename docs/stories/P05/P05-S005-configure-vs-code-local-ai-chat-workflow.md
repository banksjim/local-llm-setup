...
| Research freshness | Current VS Code and Ollama integration docs checked within 7 days. |

## 1. User story

As the workstation owner, I want this story to configure the approved Ollama or Open WebUI-backed VS Code chat path without disrupting native features, so that progress is inspectable and independent of chat memory.

## 2. Bounded objective

Configure the approved Ollama or Open WebUI-backed VS Code chat path without disrupting native features.
...
Current VS Code and Ollama integration docs checked within 7 days. Record dates, versions, direct links, claims, conflicts, and inferences; do not rely on training memory for changeable facts.
...
Preview, validate, lease, execute reversible units, stop on drift, test, record sanitized evidence, release, and route to review. A WSL repository can chat with the intended local model and restore prior settings.

## 11. Automated acceptance tests

A WSL repository can chat with the intended local model and restore prior settings. Applicable schema, scope, secret, link, static, idempotency, rollback, and dependency checks pass; justified exclusions are recorded.
