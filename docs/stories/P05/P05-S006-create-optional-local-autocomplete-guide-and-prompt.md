...
| Research freshness | Current VS Code, Continue, and Ollama documentation checked within 7 days. |

## 1. User story

As the workstation owner, I want this story to document Continue-based local autocomplete separately from VS Code native AI and provide an LLM-executable setup prompt, so that progress is inspectable and independent of chat memory.

## 2. Bounded objective

Document Continue-based local autocomplete separately from VS Code native AI and provide an LLM-executable setup prompt.
...
Current VS Code, Continue, and Ollama documentation checked within 7 days. Record dates, versions, direct links, claims, conflicts, and inferences; do not rely on training memory for changeable facts.
...
Preview, validate, lease, execute reversible units, stop on drift, test, record sanitized evidence, release, and route to review. The guide covers detection, backup, model, config, ghost-text test, conflict stop, disable, and rollback.

## 11. Automated acceptance tests

The guide covers detection, backup, model, config, ghost-text test, conflict stop, disable, and rollback. Applicable schema, scope, secret, link, static, idempotency, rollback, and dependency checks pass; justified exclusions are recorded.
