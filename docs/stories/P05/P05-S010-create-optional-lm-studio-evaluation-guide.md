...
As the workstation owner, I want this story to document a separate model copy, API port 1239, client switching, storage, test, stop, and removal workflow, so that progress is inspectable and independent of chat memory.

## 2. Bounded objective

Document a separate model copy, API port 1239, client switching, storage, test, stop, and removal workflow.
...
Preview, validate, lease, execute reversible units, stop on drift, test, record sanitized evidence, release, and route to review. Ollama and LM Studio can run identical model families without port or file-management conflict.

## 11. Automated acceptance tests

Ollama and LM Studio can run identical model families without port or file-management conflict. Applicable schema, scope, secret, link, static, idempotency, rollback, and dependency checks pass; justified exclusions are recorded.
